...
.controller("wishlistController", function($scope) {
    const CLASS_NAME = "wishlistController";
    ...
    socket.emit(socketCodes.RECORD_WISHLIST);
    ...
    socket.on(socketCodes.UPDATE_POLLER_STATUS, function(pollingStatus) {
        ...
        $scope.hideLoadingSpinner = pollingStatus.records.wishlistDone;
        var status = "ON";
        if ($scope.hideLoadingSpinner) {
            status = "OFF";
        }
        if (priorValue != $scope.hideLoadingSpinner) {
            logger.logInfo(CLASS_NAME, "The client said to turn the spinner " + status + ", updating frontend spinner");
            $scope.$apply();
        }
    });

    // Listen for server requests to remove a particular item from the records list
    socket.on(socketCodes.REMOVE_RECORD_WISHLIST_ITEM, function(itemToRemove) {
